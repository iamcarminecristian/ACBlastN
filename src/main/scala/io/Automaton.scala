...
class Automaton  {
        ...
        word.getBytes().foreach { t =>
            gotoState(trie(state), t) match {
                case -1 => val next = trie.size
                    ...
                    trie += Node(next, t, mutable.Map[Byte, Int](), 0, mutable.ArrayBuffer[(String, String)]())
                    state = next
    ...
    /**
      ...
      */
    def search(str: String): mutable.ArrayBuffer[(String, String)] = {
        val resultSet = mutable.ArrayBuffer[(String, String)]()
        var node = trie(0)
